<?php
/**
 * Copyright Ayasoftware
 */
declare(strict_types=1);

namespace Ayasoftware\ReviewApi\Model\Review\Command;

use Ayasoftware\ReviewApi\Api\Data\ReviewInterface;
use Ayasoftware\ReviewApi\Model\Converter\Review\ToModel;
use Ayasoftware\ReviewApi\Model\Converter\Review\ToDataModel;
use Magento\Framework\Exception\AlreadyExistsException;
use Magento\Framework\Exception\NoSuchEntityException;
use Magento\Review\Model\ResourceModel\Review as ReviewResource;
use Ayasoftware\ReviewApi\Validation\ValidationException;
use Ayasoftware\ReviewApi\Model\ReviewValidatorInterface;
use Magento\Review\Model\Review;
use Magento\Store\Model\StoreManagerInterface;
use Ayasoftware\ReviewApi\Model\Review\Rating\SaveHandler;
use Ayasoftware\ReviewApi\Model\Review\Command\GetInterface;

/**
 * @inheritdoc
 */
class Save implements SaveInterface
{
    /**
     * @var ToModel
     */
    private $toModelConverter;

    /**
     * @var ToDataModel
     */
    private $toDataModelConverter;

    /**
     * @var ReviewResource
     */
    private $reviewResource;

    /**
     * @var ReviewValidatorInterface
     */
    private $reviewValidator;

    /**
     * @var StoreManagerInterface
     */
    private $storeManager;

    /**
     * @var SaveHandler
     */
    private $ratingSaveHandler;

    /**
     * @var GetInterface
     */
    private $commandGet;

    /**
     * Save constructor.
     *
     * @param ReviewValidatorInterface $reviewValidator
     * @param ToModel $toModelConverter
     * @param ToDataModel $toDataModelConvert
     * @param StoreManagerInterface $storeManager
     * @param ReviewResource $reviewResource
     * @param SaveHandler $ratingSaveHandler
     * @param GetInterface $commandGet
     */
    public function __construct(
        ReviewValidatorInterface $reviewValidator,
        ToModel $toModelConverter,
        ToDataModel $toDataModelConvert,
        StoreManagerInterface $storeManager,
        ReviewResource $reviewResource,
        SaveHandler $ratingSaveHandler,
        GetInterface $commandGet
    ) {
        $this->reviewValidator = $reviewValidator;
        $this->toDataModelConverter = $toDataModelConvert;
        $this->toModelConverter = $toModelConverter;
        $this->reviewResource = $reviewResource;
        $this->storeManager = $storeManager;
        $this->ratingSaveHandler = $ratingSaveHandler;
        $this->commandGet = $commandGet;
    }

    /**
     * Save Review
     *
     * @param ReviewInterface $dataModel
     *
     * @return ReviewInterface
     * @throws ValidationException
     * @throws AlreadyExistsException
     * @throws NoSuchEntityException
     */
    public function execute(ReviewInterface $dataModel): ReviewInterface
    {
        $this->hydrateMissingUpdateData($dataModel);
        $this->updateStores($dataModel);
        $validationResult = $this->reviewValidator->validate($dataModel);

        if (!$validationResult->isValid()) {
            $msg = implode(' ', $validationResult->getErrors());
            throw new ValidationException(__($msg), null, 0, $validationResult);
        }

        $model = $this->saveReview($dataModel);
        $this->reviewResource->aggregate($model);

        return $this->toDataModelConverter->toDataModel($model);
    }

    /**
     * Save Review
     *
     * @param ReviewInterface $dataModel
     *
     * @return Review
     * @throws AlreadyExistsException
     * @throws NoSuchEntityException
     */
    private function saveReview(ReviewInterface $dataModel): Review
    {
        $model = $this->toModelConverter->toModel($dataModel);
        $this->reviewResource->save($model);
        $this->reviewResource->load($model, $model->getId());

        if ($dataModel->getStoreId() === null) {
            $dataModel->setStoreId($model->getStoreId());
        }

        $dataModel->setId($model->getId());
        $this->ratingSaveHandler->execute($dataModel);

        return $model;
    }

    /**
     * Update Review Stores
     *
     * @param ReviewInterface $dataModel
     *
     * @return void
     *
     * @throws NoSuchEntityException
     */
    private function updateStores(ReviewInterface $dataModel): void
    {
        $stores = $dataModel->getStores();

        if ($stores === null || empty($stores)) {
            $dataModel->setStores([$this->storeManager->getStore()->getId()]);
        }

        if (($dataModel->getId() === null) && ($dataModel->getStoreId() === null)) {
            $dataModel->setStoreId($this->storeManager->getStore()->getId());
        }
    }

    /**
     * Preserve the persisted review data when update requests only send a subset of fields,
     * for example approving a review by changing only review_status.
     *
     * @param ReviewInterface $dataModel
     *
     * @return void
     * @throws NoSuchEntityException
     */
    private function hydrateMissingUpdateData(ReviewInterface $dataModel): void
    {
        $reviewId = (int) $dataModel->getId();

        if (!$reviewId) {
            return;
        }

        $existingReview = $this->commandGet->execute($reviewId);

        if ($this->isBlankString($dataModel->getTitle())) {
            $dataModel->setTitle((string) $existingReview->getTitle());
        }

        if ($this->isBlankString($dataModel->getDetail())) {
            $dataModel->setDetail((string) $existingReview->getDetail());
        }

        if ($this->isBlankString($dataModel->getNickname())) {
            $dataModel->setNickname((string) $existingReview->getNickname());
        }

        if ($this->isBlankString($dataModel->getReviewEntity())) {
            $dataModel->setReviewEntity((string) $existingReview->getReviewEntity());
        }

        if (!(int) $dataModel->getEntityPkValue()) {
            $dataModel->setEntityPkValue((int) $existingReview->getEntityPkValue());
        }

        if ($dataModel->getStoreId() === null) {
            $dataModel->setStoreId((int) $existingReview->getStoreId());
        }

        if (empty($dataModel->getStores())) {
            $dataModel->setStores($existingReview->getStores() ?? []);
        }

        if ($dataModel->getCustomerId() === null && $existingReview->getCustomerId() !== null) {
            $dataModel->setCustomerId((int) $existingReview->getCustomerId());
        }

        if (!$dataModel->getReviewType() && $existingReview->getReviewType()) {
            $dataModel->setReviewType((int) $existingReview->getReviewType());
        }

        if ($dataModel->getRatings() === null && $existingReview->getRatings() !== null) {
            $dataModel->setRatings($existingReview->getRatings());
        }
    }

    /**
     * @param string|null $value
     *
     * @return bool
     */
    private function isBlankString(?string $value): bool
    {
        return trim((string) $value) === '';
    }
}
